require('dotenv').config({ path: require('path').join(__dirname, '../.env') });
const mongoose = require('mongoose');
const Artisan = require('../models/Artisan');
const Product = require('../models/Product');

const sampleArtisans = [
  {
    name: 'Aisha Okechukwu',
    story: 'Aisha is a master weaver from Nigeria, creating beautiful traditional textiles that tell stories of her ancestors.',
    imageUrl: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?w=400',
    location: 'Lagos, Nigeria',
    specialty: 'Textile Weaving',
    isActive: true
  },
  {
    name: 'Kwame Asante',
    story: 'Kwame is a skilled woodcarver from Ghana, specializing in traditional Ashanti stools and ceremonial objects.',
    imageUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400',
    location: 'Kumasi, Ghana',
    specialty: 'Wood Carving',
    isActive: true
  },
  {
    name: 'Fatima Benali',
    story: 'Fatima creates stunning ceramic pottery inspired by Moroccan geometric patterns and Islamic art.',
    imageUrl: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=400',
    location: 'Fez, Morocco',
    specialty: 'Ceramic Pottery',
    isActive: true
  },
  {
    name: 'David Mwangi',
    story: 'David is a talented jewelry maker from Kenya, working with recycled materials and traditional beadwork techniques.',
    imageUrl: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400',
    location: 'Nairobi, Kenya',
    specialty: 'Jewelry Making',
    isActive: true
  }
];

const sampleProducts = [
  {
    name: 'Traditional Yoruba Wrapper',
    price: 1400,
    imageUrl: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400',
    description: 'Handwoven cotton wrapper with traditional Yoruba patterns.',
    category: 'Textiles',
    isAvailable: true,
    stockQuantity: 15,
    artisanName: 'Aisha Okechukwu'
  },
  {
    name: 'Adire Indigo Scarf',
    price: 700,
    imageUrl: 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=400',
    description: 'Beautiful indigo-dyed scarf using traditional Adire techniques.',
    category: 'Accessories',
    isAvailable: true,
    stockQuantity: 25,
    artisanName: 'Aisha Okechukwu'
  },
  {
    name: 'Ashanti Stool',
    price: 4650,
    imageUrl: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=400',
    description: 'Hand-carved Ashanti stool with traditional symbols.',
    category: 'Furniture',
    isAvailable: true,
    stockQuantity: 8,
    artisanName: 'Kwame Asante'
  },
  {
    name: 'Ceremonial Mask',
    price: 3100,
    imageUrl: 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400',
    description: 'Intricately carved ceremonial mask representing Ashanti spiritual traditions.',
    category: 'Decor',
    isAvailable: true,
    stockQuantity: 5,
    artisanName: 'Kwame Asante'
  },
  {
    name: 'Moroccan Tea Set',
    price: 2000,
    imageUrl: 'https://images.unsplash.com/photo-1544787219-7f47ccb76574?w=400',
    description: 'Handcrafted ceramic tea set with traditional Moroccan geometric patterns.',
    category: 'Kitchenware',
    isAvailable: true,
    stockQuantity: 12,
    artisanName: 'Fatima Benali'
  },
  {
    name: 'Decorative Plate',
    price: 1240,
    imageUrl: 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400',
    description: 'Beautiful decorative plate perfect for wall display or special occasions.',
    category: 'Decor',
    isAvailable: true,
    stockQuantity: 20,
    artisanName: 'Fatima Benali'
  },
  {
    name: 'Recycled Bead Necklace',
    price: 1000,
    imageUrl: 'https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?w=400',
    description: 'Handcrafted necklace using recycled materials and traditional Kenyan beadwork.',
    category: 'Jewelry',
    isAvailable: true,
    stockQuantity: 30,
    artisanName: 'David Mwangi'
  },
  {
    name: 'Bone Carved Bracelet',
    price: 700,
    imageUrl: 'https://images.unsplash.com/photo-1605100804763-247f67b3557e?w=400',
    description: 'Elegant bracelet carved from sustainable bone materials with traditional patterns.',
    category: 'Jewelry',
    isAvailable: true,
    stockQuantity: 18,
    artisanName: 'David Mwangi'
  }
];

async function addProducts() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log('Connected to MongoDB');
  console.log('Database:', mongoose.connection.name);

  const artisansByName = {};
  for (const artisanData of sampleArtisans) {
    let artisan = await Artisan.findOne({ name: artisanData.name });
    if (!artisan) {
      artisan = await Artisan.create(artisanData);
      console.log(`Created artisan: ${artisan.name}`);
    } else {
      console.log(`Using existing artisan: ${artisan.name}`);
    }
    artisansByName[artisan.name] = artisan;
  }

  let created = 0;
  let skipped = 0;

  for (const productData of sampleProducts) {
    const existing = await Product.findOne({ name: productData.name });
    if (existing) {
      console.log(`Skipped (already exists): ${productData.name}`);
      skipped += 1;
      continue;
    }

    const artisan = artisansByName[productData.artisanName];
    const product = await Product.create({
      name: productData.name,
      price: productData.price,
      currency: 'GHS',
      imageUrl: productData.imageUrl,
      description: productData.description,
      category: productData.category,
      isAvailable: productData.isAvailable,
      stockQuantity: productData.stockQuantity,
      artisan: artisan._id
    });

    artisan.products.push(product._id);
    await artisan.save();
    created += 1;
    console.log(`Created product: ${product.name} (GH₵${product.price}) for ${artisan.name}`);
  }

  const total = await Product.countDocuments();
  console.log(`\nDone. Created ${created}, skipped ${skipped}. Total products in DB: ${total}`);
  await mongoose.disconnect();
}

addProducts().catch(async (err) => {
  console.error('Failed to add products:', err);
  await mongoose.disconnect();
  process.exit(1);
});
